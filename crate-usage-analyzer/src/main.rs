...
use analyzer::CrateUsageAnalyzer;
...
struct Args {
  ...
  #[arg(
    long,
    value_delimiter = ',',
    help = "Comma-separated list of crate names to ignore in analysis"
  )]
...
fn main() -> Result<()> {
  ...
  let workspace_path = args.workspace.clone().unwrap_or_else(|| std::env::current_dir().unwrap());

  println!("{}", format!("🔍 Analyzing workspace at: {}", workspace_path.display()).cyan().bold());
...
fn run_analyzer(args: Args, workspace_path: PathBuf) -> Result<()> {
  let entry_crate = if let Some(entry) = args.entry {
    ...
    println!("{}", format!("📦 Auto-detected entry crate: {}", entry_points[0]).green().bold());
    entry_points[0].clone()
  };

  println!("{}", format!("📦 Starting from entry crate: {}", entry_crate).green().bold());
  ...
  let report = report::generate_report(&analysis_result, &args.output_format, args.only_unused)?;

  if let Some(output_path) = args.output {
    std::fs::write(&output_path, report)?;
    println!("{}", format!("✅ Report saved to: {}", output_path.display()).green().bold());
  } else {
    println!("\n{}", "=".repeat(80).yellow());
    println!("{}", report);
  }
