...
impl SymbolGraph {
  pub fn new(entry_crate: String) -> Self {
    Self { nodes: FxHashMap::default(), entry_crate }
  }
  ...
  pub fn get_crate_symbols(&self, crate_name: &str) -> Vec<String> {
    self
      ...
      .filter_map(
        |(key, node)| {
          if node.crate_name == crate_name { Some(key.clone()) } else { None }
        },
      )
      .collect()
  }
  ...
  fn is_symbol_used(&self, node: &SymbolNode) -> bool {
    // 快速检查：内部使用、外部使用、或被重新导出
    if !node.internal_uses.is_empty()
      || !node.external_uses.is_empty()
      || !node.re_exported_by.is_empty()
    {
      return true;
    }

    // 公开 API 检查：入口 crate、次级入口 crate、或从入口暴露
    (node.is_entry_crate && node.symbol.is_public)
      || (node.is_secondary_entry && node.symbol.is_public)
      || node.is_pub_from_entry
  }
  ...
  fn get_unused_reason(&self, node: &SymbolNode) -> String {
    ...
    if node.is_secondary_entry {
      return "Secondary entry crate symbol not used internally (treated as public library API)"
        .to_string();
    }
  ...
  pub fn get_statistics(&self) -> GraphStatistics {
    ...
    let usage_percentage =
      if total_symbols > 0 { (used_symbols as f64 / total_symbols as f64) * 100.0 } else { 0.0 };
