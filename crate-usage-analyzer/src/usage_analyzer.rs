...
use std::path::Path;
use syn::visit::Visit;
use syn::{ItemUse, Path as SynPath, UseTree, ExprPath, ExprMethodCall, ExprField, TypePath, PatStruct, PatTupleStruct, Stmt, Expr, Pat, Type};
...
impl SymbolUsageAnalyzer {
    ...
    /// 批量添加要追踪的符号
    pub fn add_tracked_symbols(&mut self, symbols: Vec<(String, String)>) {
        for (symbol_name, crate_name) in symbols {
            self.tracked_symbols.insert(symbol_name, crate_name);
        }
    }
    ...
    /// 获取特定符号的使用位置
    pub fn get_symbol_uses(&self, symbol_key: &str) -> Option<&FxHashSet<String>> {
        self.detected_uses.get(symbol_key)
    }

    /// 清空分析结果
    pub fn clear(&mut self) {
        self.detected_uses.clear();
        self.use_imports.clear();
    }
