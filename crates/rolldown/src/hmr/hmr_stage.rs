...
impl<'a> HmrStage<'a> {
  ...
  pub async fn compile_lazy_entry(
    ...
    // The proxy has been marked as executed, so the lazy compilation plugin's load hook
    // will return the executed template which imports the real module.
  ...
  async fn compute_hmr_update_single(
    ...
    // Use absolute module IDs for consistent runtime lookup (in compute_hmr_update_single)
    ...
    Ok(HmrUpdate::Patch(HmrPatch {
      ...
      // Use absolute module IDs for hmr_boundaries output
  ...
  async fn render_hmr_patch_from_prerequisites(
    ...
    // Use absolute module IDs for consistent runtime lookup (in render_hmr_patch_from_prerequisites)
    source_joiner.append_source(format!(
      "__rolldown_runtime__.applyUpdates([{}]);",
      hmr_prerequisites
        .boundaries
