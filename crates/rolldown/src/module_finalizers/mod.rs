...
impl<'me, 'ast> ScopeHoistingFinalizer<'me, 'ast> {
  ...
  fn inlined_chunk_export_for(&self, canonical_ref: SymbolRef) -> Option<(&str, &str)> {
    ...
    target_chunk.inline_share_key.as_ref()?;
  ...
  fn finalized_expr_for_symbol_ref(
    ...
    if let Some(ns_alias) = namespace_alias {
      ...
      // The source calls this symbol as a plain function, where `this` is `undefined`. Reading it
  ...
  /// `experimentalInlineCommonChunks` replaces an inlined chunk's file with factory placements,
