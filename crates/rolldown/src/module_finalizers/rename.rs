  ...
  pub fn rewrite_simple_assignment_target(
    ...
    match target {
      ...
      ast::SimpleAssignmentTarget::StaticMemberExpression(member_expr) => {
        if let Some(new_target) = self.try_rewrite_cjs_member_expr_assignment_target(member_expr) {
          *target = new_target;
        }
      }
      ast::SimpleAssignmentTarget::ComputedMemberExpression(member_expr) => {
        if let Some(new_target) =
          self.try_rewrite_cjs_computed_member_expr_assignment_target(member_expr)
        {
          *target = new_target;
        }
      }
