...
impl<'me, 'ast> ScopeHoistingFinalizer<'me, 'ast> {
  ...
  fn generate_esm_namespace_in_cjs(
    &self,
  ) -> (Vec<ast::Statement<'ast>>, Vec<ast::Statement<'ast>>) {
    let mut var_decls_stmts = vec![];
    let mut inits_stmts = vec![];

    if let Some(esm_ns) = &self.ctx.module.esm_namespace_in_cjs {
      if self.ctx.module.stmt_infos[esm_ns.stmt_info_idx].is_included {
        // var import_foo;
        let var_decl = ast::Statement::VariableDeclaration(
          self.snippet.var_decl_without_init(self.canonical_name_for(esm_ns.namespace_ref)),
        );

        var_decls_stmts.push(var_decl);
        ...
        // `require_foo`
        let importee_wrapper_ref_name =
          self.finalized_expr_for_symbol_ref(self.ctx.linking_info.wrapper_ref.unpack(), false);

        // `import_foo`
        let binding_name_for_wrapper_call_ret = self.canonical_name_for(esm_ns.namespace_ref);

        // import_foo = __toESM(require_foo());
        let assign_init = self.snippet.builder.statement_expression(
          SPAN,
          self.snippet.builder.expression_assignment(
            ...
            ast::AssignmentTarget::AssignmentTargetIdentifier(
              self
                .snippet
                .builder
                .alloc_identifier_reference(SPAN, binding_name_for_wrapper_call_ret.to_string()),
            ),
            // __toESM(require_foo())
            self.snippet.wrap_with_to_esm(
              to_esm_fn_name,
