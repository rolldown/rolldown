...
use rolldown_common::{
  EcmaRelated, EntryMeta, EntryPoint, EntryPointKind, ExternalModule, ExternalModuleTaskResult,
...
pub struct ModuleLoaderOutput {
  ...
  pub emitted_entry_point_to_reference_ids: FxHashMap<EntryPoint, Vec<ArcStr>>,
  pub flat_options: FlatOptions,
}
...
impl<'a> ModuleLoader<'a> {
  ...
  pub async fn fetch_modules(
    ...
    for resolved_id in fetch_mode.iter().cloned() {
      ...
      let entry_meta = if self.cache.user_defined_entry.contains(&resolved_id.id) {
        EntryMeta::UserDefinedEntry
      } else {
        EntryMeta::empty()
      };
      ...
      self.try_spawn_new_task(resolved_id, None, entry_meta, None, &user_defined_entries);
    }
    ...
    let mut emitted_entry_points = vec![];
    let mut emitted_entry_point_to_reference_ids: FxHashMap<EntryPoint, Vec<ArcStr>> =
      FxHashMap::default();
      ...
      match msg {
        ...
        ModuleLoaderMsg::AddEntryModule(msg) => {
          ...
          let module_idx = match result {
            Ok(resolved_id) => self.try_spawn_new_task(
              resolved_id,
              None,
              EntryMeta::EmittedChunk,
              None,
              &user_defined_entries,
            ),
            Err(e) => {
          ...
          emitted_entry_point_to_reference_ids
            .entry(entry.clone())
            .or_default()
            .push(msg.reference_id.clone());

          emitted_entry_points.push(entry);
        }
    ...
    let mut module_table = if is_dense_index_vec {
    ...
    idx_of_module_info_need_update.extend(emitted_entry_points.iter().map(|item| item.idx));
    ...
    let emitted_entry_indices: FxHashSet<ModuleIdx> =
      emitted_entry_points.iter().map(|e| e.idx).collect();
    ...
    for entry_point in emitted_entry_points {
      let Some(module) = module_table.get_mut(entry_point.idx).as_normal_mut() else {
        continue;
      };
      module.entry_meta.insert(EntryMeta::EmittedChunk);
