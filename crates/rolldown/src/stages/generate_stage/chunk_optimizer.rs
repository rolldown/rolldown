...
mod tests {
  ...
  /// Helper to build a ChunkOptimizationGraph with a specific dependency structure.
  fn build_graph(chunk_deps: &[&[usize]]) -> ChunkOptimizationGraph {
    ...
    ChunkOptimizationGraph {
      ...
      module_to_chunk: IndexVec::new(),
      chunk_idx_to_temp_chunk_idx: FxHashMap::default(),
    }
  }

  #[test]
  fn test_no_false_positive_when_source_has_deps() {
    // Chunk graph:
    //   0 (target/entry) -> deps: {1, 2}
    //   1 (common/shared) -> deps: {2}     (this is the source to merge into 0)
    //   2 (common/lib)    -> deps: {}
    //
    // Merging chunk 1 into chunk 0 is safe — no cycle would be created.
    // The old algorithm (BFS from source.deps ∪ target.deps with simulation)
    // would incorrectly detect a cycle:
    //   BFS seeds: source.deps ∪ target.deps = {2} ∪ {1, 2} = {1, 2}
    //   Visit 1: deps={2}. 1.deps.contains(source=1)? YES → queue target(0)
    //   Visit 0: found target → FALSE POSITIVE!
    //
    // The fix (source has deps → BFS from source.deps only):
    //   BFS seeds: source.deps = {2}
    //   Visit 2: deps={}. Not target(0) → no cycle. CORRECT!
    let graph = build_graph(&[
      &[1, 2], // chunk 0 (target): depends on 1 and 2
      &[2],    // chunk 1 (source): depends on 2
      &[],     // chunk 2 (lib): no deps
    ]);

    let source = ChunkIdx::from_usize(1);
    let target = ChunkIdx::from_usize(0);

    // Should NOT detect a cycle — merging 1 into 0 is safe
    assert!(
      !graph.would_create_circular_dependency(source, target),
      "False positive: merging chunk 1 into chunk 0 should be safe"
    );
  }

  #[test]
  fn test_detects_real_cycle_when_source_has_no_deps() {
    // Chunk graph (mirrors #8361):
    //   0 (target/entry) -> deps: {1, 2}
    //   1 (cjs chunk)    -> deps: {2}
    //   2 (runtime/source) -> deps: {}
    //
    // Merging chunk 2 (runtime) into chunk 0 (entry) would create a cycle:
    //   After merge: 0 contains runtime. 1 depends on 2(=0 after merge).
    //   0 -> 1 -> 0 = CYCLE!
    let graph = build_graph(&[
      &[1, 2], // chunk 0 (target/entry): depends on 1 and 2
      &[2],    // chunk 1 (cjs): depends on runtime(2)
      &[],     // chunk 2 (source/runtime): no deps
    ]);

    let source = ChunkIdx::from_usize(2);
    let target = ChunkIdx::from_usize(0);

    // Should detect the cycle
    assert!(
      graph.would_create_circular_dependency(source, target),
      "Should detect cycle: merging runtime into entry creates 0->1->0"
    );
  }

  #[test]
  fn test_no_cycle_when_source_deps_dont_reach_target() {
    // Chunk graph:
    //   0 (target) -> deps: {1}
    //   1 (source) -> deps: {2}
    //   2          -> deps: {3}
    //   3          -> deps: {}
    //
    // Source's deps {2} -> {3} -> {} never reach target(0). Safe to merge.
    let graph = build_graph(&[
      &[1], // chunk 0 (target)
      &[2], // chunk 1 (source)
      &[3], // chunk 2
    ...
    assert!(
      !graph.would_create_circular_dependency(source, target),
      "No cycle: source deps don't reach target"
    );
  }

  #[test]
  fn test_real_cycle_when_source_deps_reach_target() {
    // Chunk graph:
    //   0 (target) -> deps: {1}
    //   1 (source) -> deps: {2}
    //   2          -> deps: {0}  ← reaches back to target!
    //
    // Source's deps {2} -> {0} = target. Real cycle!
    let graph = build_graph(&[
      &[1], // chunk 0 (target)
      &[2], // chunk 1 (source)
      &[0], // chunk 2 -> depends on target
    ]);
    ...
    assert!(
      graph.would_create_circular_dependency(source, target),
      "Real cycle: source deps reach target through 1->2->0"
    );
  }

  #[test]
  fn test_false_positive_scenario_with_many_entries() {
    // Simulates the pattern from large Angular apps:
    //   0 (entry/target) -> deps: {3, 4}
    //   1 (entry)        -> deps: {3, 4}
    //   2 (entry)        -> deps: {4}
    //   3 (common/source) -> deps: {4}   ← source to merge into 0
    //   4 (common/lib)    -> deps: {}
    //
    // Merging 3 into 0 is safe. The old algorithm would false-positive:
    //   BFS from {4} ∪ {3, 4} = {3, 4}
    //   Visit 3: deps={4}. 3.deps.contains(source=3)? YES → queue 0
    //   Visit 0: found target → FALSE POSITIVE!
    let graph = build_graph(&[
      &[3, 4], // chunk 0 (target/entry)
      &[3, 4], // chunk 1 (entry)
      &[4],    // chunk 2 (entry)
      &[4],    // chunk 3 (source/common)
      &[],     // chunk 4 (lib)
    ]);

    let source = ChunkIdx::from_usize(3);
    let target = ChunkIdx::from_usize(0);

    assert!(
      !graph.would_create_circular_dependency(source, target),
      "False positive: merging common chunk 3 into entry 0 should be safe"
    );
  }
}
