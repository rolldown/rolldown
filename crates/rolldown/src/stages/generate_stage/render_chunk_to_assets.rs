...
use crate::{
  ...
  types::generator::{CodegenArtifact, GenerateContext, Generator},
  utils::{
...
impl<'a> GenerateStage<'a> {
  ...
  fn create_chunk_to_codegen_ret_map(
    &self,
    chunk_graph: &ChunkGraph,
  ) -> Vec<Vec<Option<CodegenArtifact>>> {
        ...
        item
          ...
          .map(|&module_idx| {
            if let Some(module) = self.link_output.module_table.modules[module_idx].as_normal() {
              ...
              if !module.ecma_view.mutations.is_empty() {
                let mut magic_string = string_wizard::MagicString::new(&render_output.code);
                for mutation in &module.ecma_view.mutations {
                ...
                return Some(CodegenArtifact { code, map: render_output.map });
              }
              Some(CodegenArtifact { code: render_output.code, map: render_output.map })
            } else {
              None
            }
          })
