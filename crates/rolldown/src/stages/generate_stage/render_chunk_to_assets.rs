...
impl GenerateStage<'_> {
  pub async fn render_chunk_to_assets(
    ...
    inlined_common_chunks: &super::inline_common_chunks::InlinedCommonChunks,
        ...
        .flat_map(|(chunk_idx, chunk, module_id_to_codegen_ret)| {
          ...
          let ecma_chunks_future: ChunkGeneratorFuture = Box::pin(async move {
            let mut ecma_ctx = GenerateContext {
              ...
              resolved_paths,
              inline_renders,
              inline_registry_chunk: inlined_common_chunks.registry_chunk,
            };
  ...
  /// Renders every inlined chunk into its `__rd_share(...)` registration once. Hosts reuse its
  /// body and suffix, localize the registration callee, and re-render imports because output
  /// directories differ.
  ...
  fn render_inlined_chunks(
    ...
    for &chunk_idx in &inlined_common_chunks.chunks {
        ...
        module_sources
          ...
          .map(|source| {
            let rendered_exports = ctx.link_output.metas[source.module_idx]
              ...
              .iter()
              .filter(|(_, export)| {
                ctx.link_output.retained_export_symbols.contains(&export.symbol_ref)
              })
              .map(|(key, _)| key.clone())
              .collect::<Vec<_>>();
            (
      ...
      let (body, body_map) = body_joiner.join();
      let (factory, body_range) = render_inlined_chunk_factory(&ctx, &body);

      renders
        .insert(chunk_idx, InlinedChunkRender { factory, body_range, body_map, rendered_modules });
    }
    renders
  }
