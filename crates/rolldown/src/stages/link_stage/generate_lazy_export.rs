...
fn replace_lazy_expression_statement(
  ...
  ecma_ast.program.with_mut(|fields| {
    ...
    if !std::matches!(&*stmt, ast::Statement::ExpressionStatement(_)) {
      return false;
    }
    stmt.replace_with(|old| match old {
...
fn json_object_expr_to_esm(
  ...
  let transformed = ecma_ast.program.with_mut(|fields| {
    ...
    if program.body.len() != 1
      || !matches!(stmt.expression.without_parentheses(), Expression::ObjectExpression(_))
    {
      return false;
    }
