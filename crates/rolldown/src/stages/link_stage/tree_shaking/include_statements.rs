...
impl LinkStage<'_> {
  ...
  pub fn include_statements(&mut self) {
    ...
    dynamic_entries.sort_by_key(|item| self.module_table.modules[item.id].exec_order());

    dynamic_entries.retain(|entry| {
      if let Some(item) = self.retain_lived_dynamic_entries(entry, context.is_included_vec) {
  ...
  /// Note:
  /// this function will modify `dynamic_entries` in place and also return the unused dynamic
  /// import record idxs(due to limination of rustc borrow checker)  .
  fn retain_lived_dynamic_entries(
    &self,
    item: &EntryPoint,
    ...
    let is_lived = match item.kind {
      ...
      EntryPointKind::DynamicImport => {
        // dbg!(&item);
        // dbg!(&self.dynamic_import_exports_usage_map.get(&item.id));
