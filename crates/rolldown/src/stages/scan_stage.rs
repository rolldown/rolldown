...
use rolldown_common::{
  ...
  ResolvedId, RuntimeModuleBrief, ScanMode, SoucemapChainElement, SymbolRef, SymbolRefDb,
        ...
        match msg {
          SourceMapGenMsg::MagicString(v) => {
            ...
            map
              .entry(module_idx)
              .or_default()
              .push(SoucemapChainElement::Transform((plugin_idx, generated_sourcemap)));
    ...
    if !map.is_empty() {
      ...
      for (module_idx, sourcemaps) in map {
        ...
        module.sourcemap_chain.sort_by(|a, b| match (a, b) {
          (
            SoucemapChainElement::Transform((a_plugin_idx, _)),
            SoucemapChainElement::Transform((b_plugin_idx, _)),
          ) => {
            let a_order =
          ...
          (SoucemapChainElement::Load(_), SoucemapChainElement::Load(_)) => {
            std::cmp::Ordering::Equal
          }
          (SoucemapChainElement::Load(_), SoucemapChainElement::Transform(_)) => {
            std::cmp::Ordering::Less
          }
          (SoucemapChainElement::Transform(_), SoucemapChainElement::Load(_)) => {
