...
use crate::{stages::scan_stage::ScanStageOutput, type_alias::IndexAstScope};

#[derive(Default)]
pub struct ScanStageCache {
  cache: Option<ScanStageOutput>,
  module_id_to_idx: FxHashMap<ArcStr, ModuleIdx>,
}

impl ScanStageCache {
  pub fn set_cache(&mut self, cache: ScanStageOutput) {
  ...
  pub fn merge(&mut self, mut scan_stage_output: ScanStageOutput) {
    let Some(ref mut cache) = self.cache else {
      self.cache = Some(scan_stage_output);
      return;
    };
    ...
    for new_module in scan_stage_output.module_table.modules {
      ...
      let Some(new_module) = cache.module_table.modules[idx].as_normal() else {
        continue;
      };
      ...
      std::mem::swap(
        ...
        scan_stage_output.symbol_ref_db.local_db_mut(idx),
      );
    }
  }
  ...
  pub fn create_output(&mut self) -> ScanStageOutput {
