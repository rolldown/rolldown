...
pub fn render_chunk_imports(
  ...
  let mut imports_from_external_modules =
    this.imports_from_external_modules.iter().collect::<Vec<_>>();
  ...
  imports_from_external_modules.into_iter().for_each(|(importee_id, named_imports)| {
    ...
    let mut import_items = named_imports
      ...
      .filter_map(|item| {
        ...
        let alias = &this.canonical_names[&canonical_ref];
        match &item.imported {
          Specifier::Star => {
  ...
  this.imports_from_other_chunks.iter().for_each(|(exporter_id, items)| {
    ...
    let mut import_items = items
      ...
      .map(|item| {
        ...
        let local_binding = &this.canonical_names[&canonical_ref];
