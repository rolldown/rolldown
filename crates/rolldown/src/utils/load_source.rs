...
use rolldown_common::{
  ModuleType, NormalizedBundlerOptions, ResolvedId, SoucemapChainElement, StrOrBytes,
...
pub async fn load_source<Fs: FileSystem + 'static>(
  ...
  fs: Fs,
  sourcemap_chain: &mut Vec<SoucemapChainElement>,
  ...
  let (maybe_source, maybe_module_type) =
    match plugin_driver.load(&HookLoadArgs { id: &resolved_id.id }).await? {
      Some(load_hook_output) => {
        sourcemap_chain.extend(load_hook_output.map.map(SoucemapChainElement::Load));
