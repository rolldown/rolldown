  ...
  pub fn build(
    ...
    if !bundle_options.inject.is_empty() {
      ast.program.with_mut(|WithMutFields { program, allocator, .. }| {
        ...
        let ret = InjectGlobalVariables::new(allocator, inject_config).build(program);
        if ret.changed {
          scoping = None;
        }
      });
    ...
    if bundle_options.treeshake.is_some() && !has_lazy_export {
      ast.program.with_mut(|WithMutFields { program, allocator, .. }| {
        let scoping = self.recreate_scoping(&mut scoping, program);
        let mut treeshaking_options = TreeShakeOptions::from(&bundle_options.treeshake);
        treeshaking_options.invalid_import_side_effects = true;
