...
impl NestedScopeRenamer<'_, '_> {
  ...
  pub fn rename_cjs_locals_shadowing_referenced_chunk_bindings(
    ...
    // facades. A CJS-wrapped importee gets `require_x()`. A wrapped-ESM importee gets
    // `(init_x(), __toCommonJS(xxx_exports))`. A root-scope local that shares one of those final
    // names shadows the read (issue #9882, require()/namespace channel).
    //
    // Each facade has its own gate below, because the finalizer drops only the namespace half.
    // Both facades live in the importee module, so a same-named root-scope local here always
    // shadows. The one exception is a self-require, which `is_shadowable_chunk_binding` drops as
    // a known miss.
  ...
  ///
  /// Under CJS output, the finalizer reaches a cross-chunk symbol as `require_other.foo` (see
  /// `finalized_expr_for_cross_chunk_symbol`), so no local can shadow the *member* name. The base
  /// `require_other` is a plain chunk-root binding. A local can shadow it, but this gate skips it.
  /// This gate also skips a facade that this module owns (a self-`require`), although that facade
  /// renders at chunk root. Both skips are known misses.
