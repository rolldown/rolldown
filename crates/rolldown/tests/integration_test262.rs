...
// Match paths like /home/user/rolldown/test262/... or /home/runner/work/rolldown/rolldown/test262/...
static TEST262_PATH_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"[^\s]+/test262/").unwrap());
...
",
  ...
  fn evaluate(&self, output: &rolldown::BundleOutput) -> TestOutcome {
    let unique_id =
      std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos();
    let temp_dir = std::env::temp_dir().join("rolldown_test262").join(unique_id.to_string());
    ...
    match std::process::Command::new("node").arg(&wrapper_file).output() {
      Ok(node_output) => {
        ...
        self.process_node_output(&node_output, unique_id)
      }
  ...
  fn process_node_output(
    ...
    let normalized_stdout = Self::normalize_output(&stdout, unique_id);
    let normalized_stderr = Self::normalize_output(&stderr, unique_id);
  ...
  fn normalize_output(output: &str, unique_id: u128) -> String {
...
fn normalize_diagnostic_output(output: &str) -> String {
  TEST262_PATH_RE.replace_all(output, "<test262>/").into_owned()
}
