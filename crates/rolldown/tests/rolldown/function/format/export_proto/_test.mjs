...
if (configName === 'cjs') {
  ...
  assert.strictEqual(Object.hasOwn(cjs, '__proto__'), true, 'CJS: __proto__ should be an own property')
  ...
  assert.strictEqual(Object.hasOwn(iife, '__proto__'), true, 'IIFE: __proto__ should be an own property')
  ...
  assert.strictEqual(Object.hasOwn(umd, '__proto__'), true, 'UMD: __proto__ should be an own property')
  assert.strictEqual(umd['__proto__'], 123, 'UMD: __proto__ should equal 123')
} else {
  const esm = await import('./dist/main.js')
  assert.strictEqual(Object.hasOwn(esm, '__proto__'), true, 'ESM: __proto__ should be an own property')
