...
pub enum TransformOptionsInner {
  ...
  Raw(Arc<BundlerTransformOptions>),
...
impl TransformOptions {
  ...
  pub fn new_raw(
    ...
    Self { inner: TransformOptionsInner::Raw(Arc::new(bundler_options)), target, jsx_preset }
  }
  ...
  pub fn options_for_file(&self, file_path: &Path) -> Arc<OxcTransformOptions> {
    match &self.inner {
      ...
      TransformOptionsInner::Raw(opts) => {
        ...
        let Some(tsconfig) = resolver.find_tsconfig(file_path).unwrap() else {
          // TODO: Handle warnings and errors and cache below result
          return Arc::new(
            merge_transform_options_with_tsconfig(opts.deref().clone(), None, &mut vec![]).unwrap(),
          );
        };
