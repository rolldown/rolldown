...
pub struct SymbolRef {
  pub owner: EcmaModuleId,
  pub symbol: SymbolId,
}

impl From<(EcmaModuleId, SymbolId)> for SymbolRef {
  fn from(value: (EcmaModuleId, SymbolId)) -> Self {
...
impl From<(EcmaModuleId, SymbolId)> for SymbolOrMemberExprRef {
  fn from(value: (EcmaModuleId, SymbolId)) -> Self {
    Self::Symbol(SymbolRef { owner: value.0, symbol: value.1 })
  }
}

impl From<(EcmaModuleId, SymbolId, Vec<CompactStr>)> for SymbolOrMemberExprRef {
  fn from(value: (EcmaModuleId, SymbolId, Vec<CompactStr>)) -> Self {
