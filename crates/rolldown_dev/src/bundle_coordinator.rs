...
impl BundleCoordinator {
    ...
    match watch_event {
      Ok(batched_events) => {
        ...
        for batched_event in batched_events {
          ...
          {
            let path = WatchPath::from_absolute(path);
            if path.as_path().ancestors().any(|path| self.watched_files.contains(path)) {
              changed_files.insert(path.as_path().to_path_buf(), watcher_change_kind(kind));
            }
          }
        }
  ...
  async fn update_watch_paths(&self) -> BuildResult<()> {
    ...
    for watch_file in bundler.watch_files().iter() {
      ...
      let watch_file_str = path.to_string_lossy();
      if !self.watched_files.contains(path)
        && pattern_filter::filter(exclude, include, &watch_file_str, &cwd_str).inner()
      {
