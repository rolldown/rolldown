...
pub(crate) fn analyze(
  ...
  let mut ids: Vec<&str> = adjacency
    .iter()
    .flat_map(|(from, deps)| std::iter::once(from.as_str()).chain(deps.iter().map(|(to, _)| to.as_str())))
  ...
  let order: Vec<usize> =
    post_order.iter().rev().copied().filter(|&v| v != virtual_root).collect();
  ...
  while changed {
    ...
    for &v in &order {
      ...
      for &p in &preds[v] {
        ...
        new_idom = if new_idom == usize::MAX { p } else { intersect(&idom, &rpo_number, new_idom, p) };
      }
  ...
  let mut retained_bytes: Vec<u64> = (0..n + 1)
    .map(|v| if v < n && static_reachable[v] { node_bytes[v] } else { 0 })
    .collect();
  ...
  Some(GraphAnalysis { entry_modules, nodes, static_module_count, static_bytes, dynamic_only_count })
}
...
mod unit {
  ...
  fn diamond_retains_join_under_fork() {
    ...
    assert_eq!(node(&analysis, "c").idom, Some(analysis.nodes.iter().position(|n| n.id == "entry").unwrap()));
  ...
  fn chain_retains_transitively_and_dynamic_cuts() {
    ...
    let analysis = analyze(
      ...
      &weights(&[("entry", 5), ("route", 10), ("heavy", 5000), ("util", 300), ("lazy", 70), ("lazydep", 30)]),
      &["entry"],
    )
    .unwrap();
