...
use oxc::{
  ...
  ast::{
    AstBuilder, NONE,
    ast::{
      Argument, ArrowFunctionExpression, AssignmentOperator, AssignmentTarget, CallExpression,
      ClassElement, Declaration, ExportDefaultDeclarationKind, Expression, FormalParameterKind,
...
impl<'ast> AstFactory<'ast> {
  ...
  pub fn make_member_access_expr(&self, object: &str, property: &str) -> Expression<'ast> {
    Expression::from(MemberExpression::StaticMemberExpression(self.alloc_static_member_expression(
      SPAN,
      self.expression_identifier(SPAN, self.str(object)),
      self.identifier_name(SPAN, self.str(property)),
      false,
    )))
  }
