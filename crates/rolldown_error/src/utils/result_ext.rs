use crate::{BuildDiagnostic, BuildError, BuildResult};
...
pub trait ResultExt<Val> {
  ...
  fn map_err_to_unhandleable(self) -> BuildResult<Val>;
}
  ...
  Err_: StdError + Send + Sync + 'static,
{
  fn map_err_to_unhandleable(self) -> BuildResult<Val> {
    self.map_err(|err| {
      ...
      BuildError::Single(BuildDiagnostic::unhandleable_error(err))
    })
  }
}
