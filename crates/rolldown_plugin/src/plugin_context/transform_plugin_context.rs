...
use arcstr::ArcStr;
use rolldown_common::{
  ModuleIdx, PluginIdx, SoucemapChainElement, SourceMapGenMsg, SourcemapHires,
};
...
pub struct TransformPluginContext {
  ...
  sourcemap_chain: WeakRef<Vec<SoucemapChainElement>>,
    ...
    self.sourcemap_chain.with_inner(|sourcemap_chain| {
      if sourcemap_chain.is_empty() {
        ...
        match sourcemap_chain.first().expect("should have one sourcemap") {
          SoucemapChainElement::Transform((_, sourcemap))
          | SoucemapChainElement::Load(sourcemap) => sourcemap.clone(),
        }
      } else {
        let sourcemap_chain = sourcemap_chain
          ...
          .map(|element| match element {
            SoucemapChainElement::Transform((_, sourcemap))
            | SoucemapChainElement::Load(sourcemap) => sourcemap,
          })
          .collect::<Vec<_>>();
