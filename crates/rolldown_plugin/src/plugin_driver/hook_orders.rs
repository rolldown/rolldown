...
use crate::{
  HookUsage, PluginHookMeta, PluginOrder, pluginable::SharedPluginable,
  type_aliases::IndexPluginable,
};
  ...
  pub fn new(
    ...
    Self {
      order_by_build_start_meta: Self::sort_plugins_by_hook_meta(index_plugins, |i, p| {
        plugin_usage_vec[i].contains(HookUsage::BuildStart).then(|| p.build_start_meta())
      }),
      order_by_resolve_id_meta: Self::sort_plugins_by_hook_meta(index_plugins, |i, p| {
        plugin_usage_vec[i].contains(HookUsage::ResolveId).then(|| p.resolve_id_meta())
      }),
      ...
      order_by_close_bundle_meta: Self::sort_plugins_by_hook_meta(index_plugins, |i, p| {
        plugin_usage_vec[i].contains(HookUsage::CloseBundle).then(|| p.close_bundle_meta())
      }),
      order_by_watch_change_meta: Self::sort_plugins_by_hook_meta(index_plugins, |i, p| {
        plugin_usage_vec[i].contains(HookUsage::WatchChange).then(|| p.watch_change_meta())
      }),
      order_by_close_watcher_meta: Self::sort_plugins_by_hook_meta(index_plugins, |i, p| {
        plugin_usage_vec[i].contains(HookUsage::CloseWatcher).then(|| p.close_watcher_meta())
      }),
      order_by_transform_ast_meta: Self::sort_plugins_by_hook_meta(index_plugins, |i, p| {
        plugin_usage_vec[i].contains(HookUsage::TransformAst).then(|| p.transform_ast_meta())
      }),
    }
  }
