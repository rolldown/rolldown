...
impl PluginDriver {
  pub async fn render_start(&self) -> HookNoopReturn {
    for (_, plugin, ctx) in self.iter_sorted_plugin_with_context(|p| p.call_render_start_meta()) {
  ...
  pub async fn banner(
    ...
    for (_, plugin, ctx) in self.iter_sorted_plugin_with_context(|p| p.call_banner_meta()) {
  ...
  pub async fn footer(
    ...
    for (_, plugin, ctx) in self.iter_sorted_plugin_with_context(|p| p.call_footer_meta()) {
  ...
  pub async fn intro(
    ...
    for (_, plugin, ctx) in self.iter_sorted_plugin_with_context(|p| p.call_intro_meta()) {
  ...
  pub async fn outro(
    ...
    for (_, plugin, ctx) in self.iter_sorted_plugin_with_context(|p| p.call_outro_meta()) {
  ...
  pub async fn render_chunk(
    ...
    for (_, plugin, ctx) in self.iter_sorted_plugin_with_context(|p| p.call_render_chunk_meta()) {
  ...
  pub async fn augment_chunk_hash(
    ...
    let mut hash = None;
    for (_, plugin, ctx) in
      self.iter_sorted_plugin_with_context(|p| p.call_augment_chunk_hash_meta())
    {
  ...
  pub async fn render_error(&self, args: &HookRenderErrorArgs) -> HookNoopReturn {
    for (_, plugin, ctx) in self.iter_sorted_plugin_with_context(|p| p.call_render_error_meta()) {
  ...
  pub async fn generate_bundle(&self, bundle: &mut Vec<Output>, is_write: bool) -> HookNoopReturn {
    for (_, plugin, ctx) in self.iter_sorted_plugin_with_context(|p| p.call_generate_bundle_meta())
    {
  ...
  pub async fn write_bundle(&self, bundle: &mut Vec<Output>) -> HookNoopReturn {
    for (_, plugin, ctx) in self.iter_sorted_plugin_with_context(|p| p.call_write_bundle_meta()) {
