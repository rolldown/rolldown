use oxc::{
  ...
  ast::{
    NONE,
    ast::{
      Argument, AssignmentOperator, BindingPattern, BindingPatternKind, CallExpression,
      Declaration, Expression, FormalParameterKind, Statement, StaticMemberExpression,
        ...
        items.push(Argument::from(Expression::ArrowFunctionExpression(
          self.snippet.builder.alloc_arrow_function_expression(
            ...
            self.snippet.builder.function_body(SPAN, self.snippet.builder.vec(), {
              let binding_identifiers = pat.binding_identifiers();
              let mut statements = self.snippet.builder.vec_with_capacity(2);
              let mut declarations =
                self.snippet.builder.vec_with_capacity(binding_identifiers.len());
              binding_identifiers.into_iter().for_each(|id| {
                declarations.push(self.snippet.builder.variable_declarator(
                  SPAN,
                  VariableDeclarationKind::Var,
                  self.snippet.builder.binding_pattern(
                    BindingPatternKind::BindingIdentifier(id.clone_in(self.snippet.alloc())),
                    NONE,
                    false,
                  ),
                  None,
                ...
                self.snippet.builder.alloc_return_statement(
                  ...
                  Some(Expression::AssignmentExpression(
                    self.snippet.builder.alloc_assignment_expression(
                      ...
                      pat.into_assignment_target(self.snippet.alloc()),
                      await_expr,
                    ),
                  )),
                ),
