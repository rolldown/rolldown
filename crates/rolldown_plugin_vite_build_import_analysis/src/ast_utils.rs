...
impl<'a> BuildImportAnalysisVisitor<'a> {
  ...
  pub fn vite_preload_call(&self, argument: Argument<'a>) -> Expression<'a> {
    self.ast_factory.expression_call(
      SPAN,
      self.ast_factory.expression_identifier(SPAN, self.ast_factory.str("__vitePreload")),
      NONE,
      {
        ...
        items.push(Argument::from(if self.is_modern {
          self.ast_factory.expression_identifier(SPAN, self.ast_factory.str("__VITE_PRELOAD__"))
        } else {
        ...
        if append_import_meta_url {
          items.push(Argument::from(Expression::from(self.ast_factory.member_expression_static(
            ...
            self.ast_factory.expression_meta_property(
              SPAN,
              self.ast_factory.identifier_name(SPAN, self.ast_factory.str("import")),
              self.ast_factory.identifier_name(SPAN, self.ast_factory.str("meta")),
            ),
            self.ast_factory.identifier_name(SPAN, self.ast_factory.str("url")),
            false,
          ))));
        }
        items
