...
impl ResolverConfig {
  ...
  pub fn build(
    ...
    //
    // `ROLLDOWN_PACKAGE_MAP` takes precedence: an empty value disables the map, a non-empty value
    // is used as a path to the `.package-map.json` (relative paths are resolved against `cwd`).
    // When the env var is unset, the map generated by pnpm's `nodeExperimentalPackageMap`
    // (`node_modules/.package-map.json`) is auto-detected.
    let package_map = match std::env::var_os("ROLLDOWN_PACKAGE_MAP") {
      ...
      None => {
        let candidate = cwd.join("node_modules").join(".package-map.json");
