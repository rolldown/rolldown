...
impl ConfigVariant {
  pub fn apply(&self, config: &rolldown_common::BundlerOptions) -> BundlerOptions {
    ...
    if let Some(wrapped_module_treeshaking) = &self.wrapped_module_treeshaking {
      config.experimental = Some(ExperimentalOptions {
        wrapped_module_treeshaking: Some(*wrapped_module_treeshaking),
        ..config.experimental.unwrap_or_default()
      });
    }
  ...
  pub fn description(&self) -> String {
    ...
    if let Some(wrapped_module_treeshaking) = &self.wrapped_module_treeshaking {
      fields.push(format!("wrapped_module_treeshaking: {wrapped_module_treeshaking:?}"));
    }
    if let Some(pife_for_module_wrappers) = &self.pife_for_module_wrappers {
