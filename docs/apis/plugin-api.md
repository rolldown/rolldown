...
```js
// before the merge
sources: ['../node_modules/a/index.js', '../node_modules/b/index.js', '../src/main.js'];
x_google_ignoreList: [0, 1];

// after the merge, index 1 no longer exists
sources: ['main.js'];
x_google_ignoreList: [0, 1];
```

The fix is to transform the chunk in [`renderChunk`](/reference/Interface.Plugin#renderchunk) and return the new map with it. Rolldown composes the two maps, rebuilds the ignore list, and hashes the transformed code:
...
generateBundle(options, bundle) {
  for (const chunk of Object.values(bundle)) {
    ...
    // `generateMap()` returns no `x_google_ignoreList`, so the old one stays behind.
    Object.assign(chunk.map, s.generateMap({ source: chunk.fileName }));
  }
}
```
...
### Editing a Chunk's Sourcemap

Editing `chunk.map` in [`generateBundle`](/reference/Interface.Plugin#generatebundle) does not change the `.map` file that gets written, in either bundler. That file is a separate asset in the bundle by the time the hook runs. Rollup drops the edit without a word. Rolldown reads the map back only when the chunk is marked as changed. That happens when you assign `chunk.map`, or change another top-level property. An edit made in place is dropped without being validated at all.
...
This cannot fix the hash. `generateBundle` runs after hashing, so the emitted filename still carries the hash of the untransformed code.
