...
for (const [key, runs] of groups) {
  ...
  summaries.push({
    ...
    pairedSpeedupVsOrdinary:
      variant === 'ordinary'
        ...
        : hasPairedOrdinary
          ...
          : undefined,
    pairedPeakRssRatioVsOrdinary:
      variant === 'ordinary'
        ? statistics(runs.map(() => 1))
        : hasPairedOrdinary
          ? statistics(
              runs.map((run, index) => run.peakRssBytes / pairedOrdinary[index].peakRssBytes),
            )
          : undefined,
    ...
    workerMainReadyMs: optionalStatistics(
      runs.flatMap(
        (run) => run.initializationMetrics?.workers.map((worker) => worker.mainReadyMs) ?? [],
      ),
    ),
    workerMeasuredBootstrapMs: optionalStatistics(
      runs.flatMap(
        (run) =>
          run.initializationMetrics?.workers.map(
            (worker) => worker.workerBootstrap.measuredBootstrapMs,
          ) ?? [],
      ),
    ),
    poolTerminationMs: optionalStatistics(
      runs.map((run) => run.terminationMetrics?.poolTerminationMs),
    ),
