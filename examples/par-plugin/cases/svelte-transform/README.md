...
Every measured sample is a fresh Node.js process and a fresh worker pool. The runner performs one discarded fresh-process warmup per variant, rotates measured variant order, records CPU and peak RSS, and pins the exact Node binary, Rolldown commit, worktree status, host, matrix, corpus, and all raw samples.
