...
> interpretation do not describe head, and neither side can be rebuilt from
> this tree: the tokio binding lane no longer exists and the A/B harness is
> not committed. Treat every number below as a frozen record, not a claim
> about the current scheduler.
> The measurements also do not cover watcher host-timer cancellation,
...
Committed A/B results for the two native binding builds that existed at the
time of measurement:

- **tokio** — the binding-level tokio lane, which no longer exists; it was
  built at commit `d6622e8f0` and cannot be rebuilt from this tree
- **shared** — the scheduler every artifact compiles today
...
| ------ | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| tokio  | 27 (`physical * 3 / 2`)         | dedicated blocking pool, capped at **4** threads                                                                                                                             | the binding's own Tokio init at commit `d6622e8f0`; no such code path exists today            |
