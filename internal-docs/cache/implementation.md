...
| `module_idxs_by_clean_path`      | Clean path (module id with `?query`/`#fragment` stripped) → the modules addressed as `<clean path>?query`. HMR and the partial scan use it to invalidate a file's query variants together with the file (the html-proxy / `foo.vue?vue&type=...` pattern). Only ids that actually carry a query are indexed; lazy-compilation proxy ids (`?rolldown-lazy=`) are internal artifacts and stay out. |
| `module_idx_by_stable_id`        | `StableModuleId` → `ModuleIdx`, used by HMR.                                                                                                                                                                                                                                                                                                                                                     |

`module_idx_by_abs_path`, `module_idxs_by_clean_path`, and
`module_idx_by_stable_id` are **derived** — `build_module_index_maps`
(`scan_stage_cache.rs:311`) clears and rebuilds all three from the snapshot
whenever `set_snapshot` runs.
...
hits the `Invalidate` arm, flips it back to `Seen`, and re-scans. The file's
`?query` variants (via `module_idxs_by_clean_path`) are flipped to
`Invalidate` with it — their content comes from the file, so leaving them
`Seen` would reuse their stale cached copies without running any hook. The
...
3. **Per-module loop** (`:108`–`:172`):
   ...
   - Update `module_idx_by_abs_path` and `module_idxs_by_clean_path` (normal
     modules only, slash-normalized) and `module_idx_by_stable_id`.
...
| `ScanStageCache::set_snapshot`                           | `scan_stage_cache.rs:44`; called at `bundle.rs:250` and inside `update_defer_sync_data` | `snapshot` + rebuilds `module_idx_by_abs_path` / `module_idxs_by_clean_path` / `module_idx_by_stable_id`.                                                                                                                                                                           |
