...
When an eliminated dynamic facade is replaced by a namespace in its implementation chunk, the optimizer marks the internal dynamic `export *` chain with `SimulateFacadeChunk`. This reason keeps each participating namespace and its runtime external-star merge alive even if another entry flattens the same record to a chunk-level `export *`. Because `EntryLevelExternal` is record-global, `has_dynamic_exports` re-propagation still counts a marked record when its owning module's namespace is observed (`Unknown`) or simulates an eliminated facade; otherwise an unrelated invalidation seed can reach the simulated chain through its importer closure and sever an intermediate `__reExport`. Ordinary entry namespaces continue to use their chunk-level external re-export without materializing a namespace proxy chain. Runtime namespace imports synthesized for these records retain their original import attributes.
