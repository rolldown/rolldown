...
- `crates/rolldown/src/types/linking_metadata.rs` — link-owned `wrap_kind()` read by init order analysis
...
Uses `wrap_kind()` from `LinkingMetadata`; generate-stage order wrapping never changes it.
