...
When the reduced bitset would put an atom into a single purely dynamic-imported entry chunk, the reduction is not restricted by `preserveEntrySignatures`. If every dynamic importer only reads a statically-known subset of exports from the namespace (`DynamicImportExportsUsage::Partial`), the extra exports are unobservable and the atom is merged as-is, with no extra intermediate namespace. Otherwise, since we control all dynamic importers of the chunk, we generate a synthetic namespace inside the chunk, and use it as the "imported namespace" in the importer code (`import("./chunk.js").then((n) => n.<ns>)`). Extraction is refused when the entry's namespace or public chunk interface cannot be reproduced:

- an export **named** `then`, which `import()` would assimilate before the extraction callback runs. The declaring symbol's name is irrelevant — `const then = 1; export { then as ready }` puts `ready` in the namespace — and no other export name can become `then`, because the minified generator skips it and `ConflictResolver` reserves it (`THENABLE_HAZARD_EXPORT_NAME`), so an internal export named `then` deconflicts to `then$1`. The namespace's own chunk export alias cannot collide with a public one either: `deconflict_exported_names` reserves every predefined name before it hands out bundler-owned ones, so a module that itself exports `app_exports` pushes the namespace to `app_exports$1`.
- `has_dynamic_exports` from anything other than the single external-star case described below: a CommonJS module in the star chain, or a star record that does not resolve.

An ESM entry whose dynamic exports come from exactly one external star is still extracted (`dynamic_exports_are_external_stars`). The star may sit on the entry itself, or on a normal module the entry re-exports. Every hop of that chain adds its own `__reExport` call to the materialized namespace. Rollup merges only the entry's own direct stars, so it drops the names an indirect star supplies. `find_entry_level_external_module` skips the entry-level flatten for extraction-registered entries, so that record stays inside the materialized namespace as a runtime `__reExport` merge — the same shape Rollup produces with `_mergeNamespaces` — which keeps names that only exist at runtime visible to the dynamic importer, and lets an external-supplied callable `then` assimilate exactly where the unbundled module would (after the callback returns the merged namespace) rather than hijacking the extraction. Because the chunk publishes no `export *` of its own in that case, the chunk namespace itself never becomes thenable. Symbols the namespace statement declares (the external record's namespace binding, consumed by its own `__reExport`) survive the getter narrowing in `narrow_namespace_stmt_to_used_symbols`.

That last property is what makes the two remaining refusals load-bearing rather than cosmetic:

- **Non-ESM output.** The skip clears `chunk.entry_level_external_module_idx`, but `render_chunk_exports.rs` also emits the CJS/IIFE/UMD `Object.keys(...)` re-export shim from the module's own `star_exports_from_external_modules`, so the chunk would keep publishing the star beside the namespace's `__reExport` — and `Promise.resolve().then(() => require(...))` assimilates a thenable `exports` exactly the way `import()` does. The two merges do not agree on what they copy either: the shim enumerates a primitive external through `Object.keys`, while `__copyProps` skips non-objects entirely.
- **More than one external star anywhere in the chain.** ESM omits a name that two stars both export; `__copyProps` keeps whichever it copies first. Which names collide is unknowable for an external, so a second star is refused rather than merged into a namespace that can differ from the facade's. Rollup's `_mergeNamespaces` is first-wins here too, so this is stricter than Rollup rather than matching it.

Import attributes need no refusal: the generated namespace import rebuilds the record's `with` clause (`ImportAttribute::to_with_clause`).

Extraction otherwise does not depend on minified export names: the namespace is exported under its declaring symbol's name when `minifyInternalExports` is off, and the rewritten `import()` reads that name back. `preserveModules` never reaches this pass at all, since `generate_chunks` takes a separate branch that does not call `split_chunks`.
