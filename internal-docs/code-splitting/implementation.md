...
A debug assertion in `compute_cross_chunk_links` reconciles the prediction functions against the pass's derived edges on every build: emitted edges must be predicted (modulo emission-owned runtime-helper requests), and predicted-but-unemitted edges must be backed only by side-effect-free targets (pure over-prediction is benign for both consumers and unavoidable under constant inlining).
