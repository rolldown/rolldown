...
When the reduced bitset would put an atom into a single dynamic-entry chunk, the pass preserves that dynamic entry's observable namespace. The reduction is accepted only if the atom has no extra exports, its exports are already part of the dynamic entry's signature, it is runtime-only, it is the removed dynamic-entry module itself, or the extra exports are provably unobservable (#10263): for a pure dynamic entry (not also user-defined or emitted), when every `import()` of it reads a statically known name set (`DynamicImportExportsUsage::Partial`) and each used name resolves to one of the entry's non-ambiguous exports, the chunk keeps exporting those names and the atom's generated export names deconflict against them, so no recorded read can reach a merged binding. Reads of names the entry does not export keep the guard, since a generated name could otherwise shadow their source-semantics `undefined`. Otherwise the atom stays separate so `import("./entry.js")` does not expose helper exports needed only by other chunks. (Rollup instead accepts the reduction unconditionally and repairs the namespace at render time with a facade or an exported synthetic namespace plus `.then(n => n.ns)` call-site rewrites; Rolldown keeps the decision at assignment time.)
