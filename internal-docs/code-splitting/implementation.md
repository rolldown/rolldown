...
When the reduced bitset would put an atom into a single purely dynamic-imported entry chunk, the reduction is not restricted by `preserveEntrySignatures`. Say every dynamic importer only reads a statically-known subset of the namespace's exports (`DynamicImportExportsUsage::Partial`). The extra exports are then unobservable, so the atom is merged as-is, with no intermediate namespace. Otherwise we generate a synthetic namespace inside the chunk, since we control all of its dynamic importers. That namespace is what the importer code reads (`import("./chunk.js").then((n) => n.<ns>)`).

We bail out only when the entry's namespace could carry a callable `then`. `import()` assimilates such a namespace before the extraction callback runs. Two sources qualify:

- An export **named** `then`. The declaring symbol's name does not matter: `const then = 1; export { then as ready }` puts `ready` in the namespace. No other export name can become `then` either. The minified generator skips it and `ConflictResolver` reserves it (`THENABLE_HAZARD_EXPORT_NAME`), so an internal export named `then` deconflicts to `then$1`.
- `has_dynamic_exports` from anything but the entry's own direct `export * from '<external>'` records. That means a CommonJS star, or a star into a normal module that itself has dynamic exports.

An entry is still extracted when its dynamic exports come **only** from its own direct external stars (`dynamic_exports_are_direct_external_stars`). `find_entry_level_external_module` skips the entry-level flatten for extraction-registered entries. Those records therefore stay inside the materialized namespace, merged at run time by `__reExport`. Rollup produces the same shape with `_mergeNamespaces`. That keeps names which only exist at run time visible to the dynamic importer. A callable `then` from the external also assimilates where the unbundled module would. That happens once the callback returns the merged namespace, so it does not hijack the extraction. The chunk publishes no `export *` of its own here, so the chunk namespace itself never turns thenable. The namespace statement also declares symbols of its own, such as the external record's namespace binding. Its own `__reExport` consumes that binding, so it survives the getter narrowing in `narrow_namespace_stmt_to_used_symbols`.

Extraction does not depend on minified export names. With `minifyInternalExports` off, the namespace is exported under its declaring symbol's name, and the rewritten `import()` reads that name back. `preserveModules` never reaches this pass at all, because `generate_chunks` takes a separate branch that does not call `split_chunks`.
