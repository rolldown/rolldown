...
2. **Namespace extraction fallback.** Otherwise — the namespace is observed opaquely, or a read targets an unexported name — the merge is still accepted for a pure dynamic entry by routing it through the simulated-facade machinery that eliminated facade chunks already use: the entry is registered in `common_chunk_exported_facade_chunk_namespace`, its namespace statement is narrowed to link-retained getters, the namespace/wrapper symbols are included (`SymbolIncludeReason::SimulatedFacadeChunk`, with `__exportAll` demand recorded before standalone runtime extraction so placement stays normal), and every `import()` call site extracts it via `rewrite_dynamic_import_for_merged_entry` (`.then(n => n.<alias>)`). This is Rollup's `generateFacades` fallback (`Chunk.ts` includes `module.namespace` in the chunk and exports it) expressed at assignment time. Extraction is refused when the simulated namespace cannot reproduce the entry's interface: `has_dynamic_exports` (CJS `export *`, a star chain reaching an external — a format-dependent merge that could even supply a callable `then` at runtime) or a statically known `then` export, which would make the chunk namespace thenable and hand the extraction callback the assimilated value.

Only when both tiers refuse does the atom stay separate, so `import("./entry.js")` never exposes helper exports needed only by other chunks. Regression coverage: `optimization/chunk_merging/already_loaded_{opaque_namespace,unexported_read}_extraction/` pin the fallback, `already_loaded_{then_export,external_star}_keeps_split/` pin the refusals, and `issues/10263/` pins the lean tier.
