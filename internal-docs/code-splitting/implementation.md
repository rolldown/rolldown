...
When the reduced bitset would put an atom into a single dynamic-entry chunk, the pass preserves that dynamic entry's observable namespace, in two tiers (#10263):

1. **Lean acceptance (no extra machinery).** The reduction is accepted as-is if the atom has no extra exports, its exports are already part of the dynamic entry's signature, it is runtime-only, it is the removed dynamic-entry module itself, or the extra exports are provably unobservable: for a pure dynamic entry (not also user-defined or emitted), when every `import()` of it reads a statically known name set (`DynamicImportExportsUsage::Partial`) and each used name resolves to one of the entry's non-ambiguous exports, the chunk keeps exporting those names and the atom's generated export names deconflict against them, so no recorded read can reach a merged binding.
2. **Namespace extraction fallback.** Otherwise — the namespace is observed opaquely, or a read targets an unexported name — the merge is still accepted for a pure dynamic entry by routing it through the simulated-facade machinery that eliminated facade chunks already use: the entry is registered in `common_chunk_exported_facade_chunk_namespace`, its namespace statement is narrowed to link-retained getters (symbols the statement itself declares, like an external star record's namespace binding, are structural and survive), the namespace/wrapper symbols are included (`SymbolIncludeReason::SimulatedFacadeChunk`, with `__exportAll` demand recorded before standalone runtime extraction so placement stays normal), and every `import()` call site extracts it via `rewrite_dynamic_import_for_merged_entry` (`.then(n => n.<alias>)`). This is Rollup's `generateFacades` fallback (`Chunk.ts` includes `module.namespace` in the chunk and exports it) expressed at assignment time. An entry whose dynamic exports come entirely from its own direct `export * from <external>` records is also accepted: `find_entry_level_external_module` skips the entry-level flatten for extracted entries, so the materialized namespace keeps those records as runtime `__reExport` merges (Rollup's `_mergeNamespaces`) — names only known at runtime, including a callable `then`, land on the extracted namespace and assimilate exactly where the unbundled module would — while the suppressed chunk-level `export *` keeps the chunk namespace itself non-thenable. Extraction is still refused when the simulated namespace cannot reproduce the entry's interface: other `has_dynamic_exports` sources (a CJS `export *`, or a star chain that only reaches an external transitively) and statically known `then` exports, which would make the chunk namespace thenable and hand the extraction callback the assimilated value.

Only when both tiers refuse does the atom stay separate, so `import("./entry.js")` never exposes helper exports needed only by other chunks. Regression coverage: `optimization/chunk_merging/already_loaded_{opaque_namespace,unexported_read,external_star,external_star_then}_extraction/` pin the fallback, `already_loaded_{then_export,cjs_star}_keeps_split/` pin the refusals, and `issues/10263/` pins the lean tier.
