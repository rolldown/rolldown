...
Both entry reachability and the acyclicity check use the static imports that will actually be emitted. `predicted_static_import_targets` combines referenced symbol owners, side-effectful direct imports, the side-effectful runtime flag, and — for entry modules — entry-export service targets. It filters transitive side-effect dependencies propagated through a `sideEffects: false` barrel because those do not become imports of the barrel's chunk; retaining them would invent cycles (`optimization/chunk_merging/already_loaded_side_effectful_barrel`).

Entry-export service targets exist because `register_entry_export_depended_symbols` imports the canonical symbol of every live resolved export of an entry chunk, even when no included statement of the entry references it — an unused re-export is still served by a real import of the owner's chunk. These targets come from the full `resolved_exports` (plus re-exported ESM wrapper inits), gated by the same symbol liveness emission applies. The gate keeps the prediction exact in both directions: a missed live export lets the cycle check approve a fold that emits a cyclic graph (`optimization/chunk_merging/already_loaded_entry_reexport_service_edge`), while an added dead export would let entry reachability claim chunks the entry never loads and skip their side effects.
