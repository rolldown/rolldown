...
When the reduced bitset would put an atom into a single purely dynamic-imported entry chunk, the reduction is not restricted by `preserveEntrySignatures`. Since we control all dynamic importers of the chunk, we instead generate a synthetic namespace inside the chunk, and use it as the "imported namespace" in the importer code (`import("./chunk.js").then((n) => n.<ns>)`). We bail out in case of a `then` export, or of a dynamic/external `export * from`. The rewrite also requires bundler-controlled export naming, so it is disabled under `preserveModules` and when `minifyInternalExports` is off.
