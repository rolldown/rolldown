...
> **The "why" — the rebuild contract, error-flow, and the four design
> principles** that govern when the engine rebuilds live in
> [design.md](./design.md). This file is the implementation map. Below,
> "Design principle N" refers to design.md; "§N" refers to a section here.
...
committed. Inside the transaction the coordinator adds each path not yet in its
`watched_files` set that passes `watch.include` / `watch.exclude`, skips a
...
| `register_client(client_id)`                     | hello: new `ClientSession`, empty ship map, boot-evaluated map frozen in (`dev_engine.rs:395`) |
| `remove_client(client_id)`                       | disconnect: drops the session and its pending payloads (`dev_engine.rs:407`)                   |
| `notify_payload_delivered(filename)`             | delivery ack: max-merges the pending payload's stamps into `shipped[C]` (`dev_engine.rs:421`)  |
...
Used by: `ensureLatestBuildOutput` (`binding_dev_engine.rs:450-474`,
unwrapped in `packages/rolldown/src/api/dev/dev-engine.ts:217`) and the
lifecycle methods `run`, `ensureCurrentBuildFinish`, `compileEntry`, and
`close`. `getBundleState` and `triggerFullBuild` still convert their errors
to a plain `napi::Error` with a reason string. The thrown error reaches
