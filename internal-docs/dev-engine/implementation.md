...
2. **Drain + deliver.** `FileEmitter::add_additional_files` moves
   not-yet-delivered entries out of `files` into `Vec<Output>`, marking each in
   the `emitted_files` set so every asset is delivered **exactly once**. The
   drained outputs leave through one of three callbacks.
...
| `on_additional_assets` | `generate_hmr_updates()` **and** `compile_lazy_entry()` | `Vec<Output>` — emitted assets only                                     |
...
inside a `generateBundle` hook (that hook does not run for patches).
