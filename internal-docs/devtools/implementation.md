...
The `devtools` option is `@experimental`. Setting `devtools: {}` is enough to enable tracing. The option flows through the binding layer as `BindingDevtoolsOptions` and normalizes to `DevtoolsOptions { session_id: Option<String> }` on the Rust side. `ClassicBundler` keeps its constructor-generated ID when `sessionId` is omitted, or replaces it with the requested value before creating the tracer and session span. That selected ID is therefore shared by emitted event fields, output-directory naming, and the acknowledged close flush.
