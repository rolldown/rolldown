...
(`module_finalizers`) and the HMR finalizer's nested-import rewrite both go through
`create_request_lazy_call`, which receives the endpoint computed by
`crate::hmr::utils::lazy_endpoint_url` (trailing slashes of `base` are trimmed; no `base`
means the historical root-absolute path).
...
- `executed_modules` is a **grow-only** set of **stable ids**. Proxy ids like `src/foo.js?rolldown-lazy=1` never appear in it: nothing registers under a proxy id in the browser (see design.md "The `requestLazy` Entry Point")
...
│    (dedup flag); registrations only, no execute-entry tail              │
└─────────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────────┐
