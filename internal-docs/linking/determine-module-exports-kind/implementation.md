...
`NormalizeLazyExportsPass` performs the final draft-to-final transition. `DetermineModuleSideEffectsPass` is the last typed reader of final wrappers, but not of sealed dynamic exports. `CollectResolvedExportsPass` then reads the identity-stable final module table and returns owned raw maps. The current representation helper projects side effects, post-lazy formats, dynamic-export bits, and wrapper declarations into unchanged legacy fields before collection; later unmigrated readers require the projections, but their early position is transitional. The driver retains final `ModuleFormats`, sealed `DynamicExports`, and sealed `ModuleSideEffects` through `BindImportsPass`; projection does not consume those typed artifacts. Binding also borrows the raw resolved-export draft and sealed execution orders while owning initial dependencies and symbols. `FinalizeResolvedExportsPass` consumes the draft afterward, `ComputeCjsRoutingPass` performs the last typed format read, and `ResolveMemberExpressionsPass` performs the last typed dynamic-export and side-effect reads before the driver projects member and resolved-export artifacts.
...
- `DetermineModuleSideEffectsPass`, which borrows final `ModuleWrappers` and sealed `DynamicExports`, preserves the legacy ordered recursion and cache, and returns sealed dense `ModuleSideEffects`. The compatibility projection happens early, but the typed artifact remains available for binding.
...
- `FinalizeResolvedExportsPass`, which consumes the draft after binding's last Link-stage symbol link and produces paired raw and sorted canonical maps. The final artifact survives through member-expression resolution before one checked no-clone projection.
