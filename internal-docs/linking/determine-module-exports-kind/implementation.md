...
`NormalizeLazyExportsPass` performs the final draft-to-final transition. `DetermineModuleSideEffectsPass` is the last typed reader of final wrappers and sealed dynamic exports. The driver first projects its sealed result into normal module side-effect fields, then projects post-lazy formats, dynamic-export bits, and wrapper declarations into the unchanged legacy module and metadata fields before converting the still-typed CJS namespace, entry, and constant artifacts for their remaining legacy consumers.
