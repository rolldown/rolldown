...
final ModuleFormats + sealed DynamicExports + sealed ModuleSideEffects + ResolvedExportsDraft
  ...
  → CreateSyntheticExportStatementsPass → ReferenceNeededSymbolsPass
```

`NormalizeLazyExportsPass` performs the final draft-to-final transition. `DetermineModuleSideEffectsPass` first reads final wrappers and sealed dynamic exports, and `CollectResolvedExportsPass` then reads the identity-stable final module table and returns owned raw maps. The current representation helper projects side effects, post-lazy formats, dynamic-export bits, and wrapper declarations into unchanged legacy fields before collection; later unmigrated readers require the projections, but their early position is transitional. The driver retains final `ModuleFormats`, final `ModuleWrappers`, sealed `DynamicExports`, and sealed `ModuleSideEffects` through N; projection does not consume those typed artifacts. Binding also borrows the raw resolved-export draft and sealed execution orders while owning initial dependencies and symbols. `FinalizeResolvedExportsPass` consumes the draft afterward, M reads dynamic exports and side effects, entry-root collection reads wrappers, synthetic statement creation reads formats, and `ReferenceNeededSymbolsPass` is the last typed reader of all four representation facts before they are dropped.
