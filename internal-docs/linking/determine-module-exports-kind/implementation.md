...
`NormalizeLazyExportsPass` now performs the final draft-to-final transition. The driver projects post-lazy formats, dynamic-export bits, and wrapper declarations into the unchanged legacy module and metadata fields only after that pass finishes, then converts the still-typed CJS namespace, entry, and constant artifacts for their remaining legacy consumers.
