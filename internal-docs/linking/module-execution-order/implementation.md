...
1. **Runtime precedes wrapped modules.** A wrapper definition calls a runtime helper such as `__commonJSMin` or `__esmMin`, so the runtime receives an execution order before the first wrapped module. The runtime's unwrapped ESM dependencies retain their dependency-first order: an injected polyfill can initialize `Object.create` before the runtime captures it. With no plugin-added imports, the runtime is first.
...
3. **Dependencies execute before dependents, along acyclic edges.** For any non-back edge `A → B` that `sort_modules` traverses, `B.exec_order < A.exec_order`, except for the runtime bootstrap rule above. Back edges in cycles are another exception: when the algorithm revisits an already-executed ancestor it is skipped (that's where cycle detection fires), so a module on a cycle can receive its `exec_order` before a transitive dependency further along the cycle. Downstream stages must not assume strict topological order across cycles.
...
On its first real visit, a `ModuleIdx` goes through the `ToBeExecuted → WaitForExit` pair: popped as `ToBeExecuted` (pre-order), its own `WaitForExit` (post-order sentinel) is pushed, then dependencies are pushed above it. Their traversal completes before the module receives its `exec_order`; back edges follow the cycle rule above. The runtime can receive its order early when a wrapped dependency needs its helper. Later incoming edges may push additional `ToBeExecuted(id)` entries for the same module (e.g. the second importer in a diamond); these are popped and short-circuited by the `executed_ids` membership check rather than re-entering the pre/post pair. The complexity section below spells out the consequences for the total push count.

### Seeding the stack

The stack contains reversed entry roots followed by the runtime. A `Vec` stack pops from the end, so it traverses the runtime and its dependencies before the entries. Dependencies and entries retain their source order through reversed pushes.
...
- Otherwise, `id` is inserted into `executed_ids` and a `WaitForExit(id)` sentinel is pushed. Its import records are filtered and pushed in reverse:
  ```rust
  rec.kind.is_static()
...
On `WaitForExit(id)`, remove the stack-index bookkeeping entry. If the module has a wrapper and the runtime has no order yet, assign the runtime the next order first. The runtime's own sentinel skips reassignment if that happened. Otherwise assign `module.exec_order = next_exec_order`, push onto `sorted_modules` if it's a `Module::Normal`, and increment the counter. External modules get an `exec_order` but do not appear in `sorted_modules` (only normal modules are emitted by the chunk pipeline).
...
Because so many later stages key off `exec_order`, any change to the traversal rules here is an observable output change across the entire bundler. `sort_modules` runs after `wrap_modules` so it knows which definitions need runtime helpers, and before any stage that consumes execution order.
...
- Every module preceding the runtime in `sorted_modules` has `WrapKind::None` (asserted at the end of the pass).
...
- [runtime-helpers](../../runtime-helpers/implementation.md) — helpers that wrapped modules need before their definitions execute
