...
3. **A hook's answer is final.** Rolldown recovers metadata for the exact id the hook returned.
   It applies no alias or other rewrite to that id, and it does nothing when the hook opts out.
