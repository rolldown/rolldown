...
3. `Split Debug Info` runs `scripts/misc/split-debuginfo.mjs`. Per format:
   ...
   | ----------------------- | --------------------------------------------- | ------------------------------------------------------------------- |
   | ELF                     | `objcopy --only-keep-debug`                   | `objcopy --strip-all`, then `--add-gnu-debuglink` to the debug file |
...
gh run list --workflow publish-to-pkg.pr.new.yml --branch "$branch" --json databaseId,headSha,conclusion
