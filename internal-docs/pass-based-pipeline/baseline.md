...
`diagnostic-order` is digest-only: 12 files, 909 bytes, input digest `9586764cf0f88b8c207e13aaf4b452d9`, and 13 linked modules including runtime. It contains two ordered circular-dependency warnings, two require-TLA errors, and two missing-export errors. The pre-Generate stream must contain all six in production order; final Generate failure must contain the four errors in production order.
...
- For every digest workload and component field, require one unique value across all forty fresh processes and equality between Rayon 1 and Rayon 4. Descriptor arrays must also be identical so order and spans are not hidden behind hashes.
...
The clean runner commit, behavior corpus, timing, trace attribution, digest uniqueness, process-RSS distributions, and derived Phase 2–5 budgets are filled here only after every acceptance gate above passes.
