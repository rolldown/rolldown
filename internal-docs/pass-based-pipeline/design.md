...
`CollectResolvedExportsPass` is a concrete boundary case: its maps ultimately belong to Generate through legacy metadata, so `ResolvedExportsDraft` uses `OutputOwned`. Putting the table in `OutputRead` would force either a general unsealing escape or a graph-sized clone at the compatibility boundary. The owned artifact instead exposes one consuming slot conversion, and the driver moves each map exactly once.
