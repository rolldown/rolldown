...
The driver retains `ModuleDependenciesDraft`, sealed execution orders, final formats, sealed dynamic exports, sealed side effects, and the resolved-export draft through `BindImportsPass`. That pass owns symbols and dependencies, borrows the other facts, preserves the existing serial immediate commit order, and returns a one-call envelope that the driver destructures at once. Its private external binding groups are committed to facade symbols before it returns. `FinalizeResolvedExportsPass` then consumes the draft after the last production `SymbolRefDb::link` call in Link and pairs every raw map with a sorted canonical non-ambiguous map. Legacy member-expression resolution borrows that final artifact; only afterward does the driver move both maps into metadata without cloning. Generate has separate later code-splitting symbol links outside this Link boundary.

The entry plan, CJS namespace merges, and global constants remain typed until lazy normalization has excluded their identity owners from pristine rebuilding. The lazy pass also emits two purpose-specific sparse artifacts rather than widening a carrier: `NonSplittableJsonDefaults` survives until legacy member resolution finishes, while `LazyJsonExportInitializers` survives only until Generate materializes retained snapshot bindings immediately after their payload statements. Some compact facts are projected early for unmigrated legacy readers, but the typed value remains live until its declared final consumer. Those projections move to the final adapter as later legacy readers migrate. Import binding still combines analysis and immediate commit; a pure event plan and a separate deterministic commit step remain future work.
...
- The production link pass subtree forbids unsafe code. Its non-vacuous AST inventory requires every `XxxPass` to be one `pub(super)` non-generic unit struct with one direct unqualified `Pass` implementation, rejects aliases and hidden local declarations, limits derives and production macros to a closed exact-path allowlist, recursively inspects allowed macro arguments, and rejects the broad link carriers in pass code. Binding's preserved tracing and missing-export formatting macros are admitted only at their fully qualified paths in `bind_imports.rs`; neighboring paths and bare names remain rejected. The only conditional production attribute is the exact private `#[cfg(target_family = "wasm")] use rolldown_utils::rayon::IteratorExt as _;` compatibility import; the inventory parses that complete item and rejects other targets, paths, visibility, or absolute-path variants.
