...
- **Watch registration transactions are always finalized** — Opening a filesystem-watcher path transaction can pause event delivery until commit, including on macOS FSEvents. Every candidate addition is attempted and every opened transaction is committed even after individual additions fail. Successfully staged paths are published only after commit succeeds; add and commit failures are aggregated. A failed registration attempt is retried by the coordinator with short backoff while preserving one public build-event cycle. Failed hidden bundle attempts are closed before replacement. Persistent failure terminates through the normal replayable close-error path instead of leaving an unwakeable idle watcher.
