...
- **Shared vs per-group FsWatcher** — Output tasks of one config now share a config-group `FsWatcher` (see implementation.md), so a multi-output config watches each file once. Two _configs_ watching the same file still watch it twice at the OS level. A single shared `FsWatcher` at the coordinator level would deduplicate cross-config watches and use fewer OS resources. Adding files is straightforward. Unwatching (not yet implemented) would require cross-task coordination — a file can only be unwatched when no task needs it (reference counting or a union check across task watch sets). Since unwatching isn't implemented yet, a fully shared watcher would be strictly simpler today.
