...
- **Close is re-entrant** — A watcher event listener may call and await `watcher.close()`. Native cleanup and listener completion are separate memoized phases. Node uses `AsyncLocalStorage` and active invocation ancestry to let nested or mutually closing listeners await the completed native phase while unrelated callers await the full phase including every close listener. Browser hosts have no equivalent async-context primitive: while a close listener promise is active, same-watcher `close()` calls receive the native phase to prevent self-deadlock. Calls made before listener dispatch or after it completes still await the full phase; unrelated calls made during the listener are indistinguishable and may resolve after native cleanup. Watch build results use an opaque per-plugin-driver close identity and a process-wide, invocation-owned dependency graph: a `closeBundle` hook may await the same underlying `event.result.close()` that invoked it, including through a nested or cross-watcher A -> B -> A close cycle after async suspension, while a close for a result absent from the active invocation ancestry must await and replay that result's terminal success or failure normally. In browser hosts, an unrelated close of that same result during the active hook is likewise indistinguishable and receives the acknowledgement.
