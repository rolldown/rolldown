...
- **Close is re-entrant** — A watcher event listener may call and await `watcher.close()`. Native cleanup and listener completion are separate memoized phases. Node uses `AsyncLocalStorage`; browser hosts use a configured provider or native `AsyncContext.Variable` when available. Active invocation ancestry lets nested or mutually closing listeners await the completed native phase while unrelated callers await the full phase including every close listener. A browser host without async-context support falls back to returning the native phase to same-watcher calls while a close listener promise is active; calls made before listener dispatch or after it completes still await the full phase, but unrelated calls made during the listener are indistinguishable and may resolve after native cleanup. Watch build results use an opaque per-plugin-driver close identity and a process-wide, invocation-owned dependency graph: a `closeBundle` hook may await the same underlying `event.result.close()` that invoked it, including through a nested or cross-watcher A -> B -> A close cycle after async suspension, while a close for a result absent from the active invocation ancestry must await and replay that result's terminal success or failure normally. Browser hosts without async-context support likewise cannot distinguish an unrelated close of that same result during the active hook and return the acknowledgement.
- **Close is best-effort and terminal** — Shutdown attempts `closeWatcher` and `closeBundle` for every watch task, then emits `close`, even when an earlier phase fails. Failures are aggregated and the terminal result is replayed to concurrent and later callers; cleanup hooks are not retried after a partial hook chain may already have run.
- **Watch registration transactions are always finalized** — Opening a filesystem-watcher path transaction can pause event delivery until commit, including on macOS FSEvents, so a transaction opens only when a task has paths new to its whole config group: candidates a sibling already registered are adopted from the group's shared registered-path set without a backend attempt, and a batch with nothing group-new opens no transaction at all. Every opened transaction is committed even after individual additions fail. Successfully staged paths are published to both the task set and the group set only after commit succeeds; add and commit failures are aggregated. A failed registration attempt is retried by the coordinator with short backoff while preserving one public build-event cycle. Failed hidden bundle attempts are closed before replacement. Persistent failure terminates through the normal replayable close-error path instead of leaving an unwakeable idle watcher.
...
- **Shared vs per-group FsWatcher** — Output tasks of one config now share a config-group `DynFsWatcher` (see implementation.md), so a multi-output config watches each file once. Two _configs_ watching the same file still watch it twice at the OS level. A single shared `DynFsWatcher` at the coordinator level would deduplicate cross-config watches and use fewer OS resources. Adding files is straightforward. Unwatching (not yet implemented) would require cross-task coordination — a file can only be unwatched when no task needs it (reference counting or a union check across task watch sets). Since unwatching isn't implemented yet, a fully shared watcher would be strictly simpler today.
