...
Candidate reductions are applied and validated as a batch against the static graph formed by already-materialized manual chunks plus synthetic automatic chunks keyed by the reduced bitsets. "Already loaded" does not always mean "already initialized": if a reduced atom is moved into a chunk that statically imports one of its consumers, an ES module cycle can expose uninitialized bindings, including CJS wrapper functions.
