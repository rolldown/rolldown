...
- **Change an entry's export signature** — when `preserveEntrySignatures: 'strict'`/`'exports-only'`, `can_merge_without_changing_entry_signature` rejects a merge that would surface exports the entry doesn't already declare. It still permits the merge when every merged module either re-exports existing entry symbols _or_ keeps its exports **internal** after the merge — i.e. it is not itself an entry (user or dynamic), not dynamically imported, and every static importer already lands in the merged chunk — so folding it in can't widen the entry's public signature. This matches Rollup, which folds such modules into the entry even under `exports-only`. That internal-export relaxation (the function's `allow_internal_only` flag) is enabled only by `try_insert` (plain common chunks only); the facade pass and the already-loaded reduction pass `false`, so user-directed manual/advanced-chunk groups are never dissolved.
