...
- **Change an entry's export signature** — when `preserveEntrySignatures: 'strict'`/`'exports-only'`, merging is rejected if it would surface exports the entry doesn't already declare. `can_merge_into_strict_entry` still permits the merge when every merged module either re-exports existing entry symbols *or* keeps its exports **internal** after the merge — i.e. it is not itself an entry (user or dynamic), not dynamically imported, and every static importer already lands in the merged chunk — so folding it in can't widen the entry's public signature. This matches Rollup, which folds such modules into the entry even under `exports-only`. The relaxation is scoped to `try_insert` (plain common chunks only); the facade pass and the already-loaded reduction keep the stricter `can_merge_without_changing_entry_signature`, so user-directed manual/advanced-chunk groups are never dissolved.
