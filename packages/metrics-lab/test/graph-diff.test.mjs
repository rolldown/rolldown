...
test('+1 not +8: a new module reusing existing deps attributes only itself', () => {
  ...
  const before = makeGraph([
    ...
    { id: 'core', bytes: 100, imports: [[2, false], [3, false], [4, false]] },
    { id: 'd1', bytes: 10, imports: [] },
    { id: 'd2', bytes: 10, imports: [] },
    { id: 'd3', bytes: 10, imports: [] },
  ], ['entry']);
  const after = makeGraph([
    { id: 'entry', bytes: 5, imports: [[1, false], [5, false]] },
    { id: 'core', bytes: 100, imports: [[2, false], [3, false], [4, false]] },
    { id: 'd1', bytes: 10, imports: [] },
    { id: 'd2', bytes: 10, imports: [] },
    { id: 'd3', bytes: 10, imports: [] },
  ...
  assert.deepEqual(diff.entered.map((m) => m.id), ['T']);
  assert.equal(diff.enteredBytes, 42);
  assert.deepEqual(diff.added.map((m) => m.id), ['T']);
...
test('module leaving eager via a defer; graph-diff agrees with what-if', () => {
  ...
  assert.deepEqual(
    diff.left.map((m) => m.id).sort(),
    predicted.removed.map((m) => m.id).sort(),
  );
...
test('edge retarget: leaves detach, enter, and the edge is flagged', () => {
  ...
  const diff = diffGraphs(before, after);
  assert.deepEqual(diff.entered.map((m) => m.id), ['y']);
  assert.deepEqual(diff.left.map((m) => m.id), ['x']);
  assert.deepEqual(diff.edgesChanged.map((m) => m.id), ['a']);
...
test('renamed id reads as one add + one remove (no rename detection)', () => {
  ...
  const diff = diffGraphs(before, after);
  assert.deepEqual(diff.added.map((m) => m.id), ['src/new.ts']);
  assert.deepEqual(diff.removed.map((m) => m.id), ['src/old.ts']);
  assert.deepEqual(diff.entered.map((m) => m.id), ['src/new.ts']);
  assert.deepEqual(diff.left.map((m) => m.id), ['src/old.ts']);
});
