...
const bindingFile = nodePath.resolve('dist/binding.cjs');
const bindingFileWasi = nodePath.resolve('dist/rolldown-binding.wasi.cjs');
const bindingFileWasiBrowser = nodePath.resolve('dist/rolldown-binding.wasi-browser.js');
...
(async () => {
  ...
  // Don't delete the entire dist directory to avoid Windows permission issues
  // Only delete files that will be regenerated by the build process
  if (fs.existsSync(buildMeta.buildOutputDir)) {
    const distFiles = fs.readdirSync(buildMeta.buildOutputDir);

    // Files to preserve: binding files, .node files, .wasm files
    ...
    const shouldPreserve = (file: string) =>
      preservePatterns.some((pattern) =>
        pattern.startsWith('.') ? file.endsWith(pattern) : file === pattern,
      );

    // Delete generated files (.mjs, .d.mts, .d.ts, etc.) but preserve binding files
    for (const file of distFiles) {
      if (!shouldPreserve(file)) {
        const filePath = nodePath.join(buildMeta.buildOutputDir, file);
        try {
          const stat = fs.statSync(filePath);
          if (stat.isDirectory()) {
            // Remove subdirectories (like 'shared/')
          ...
          console.warn(`Warning: Could not delete ${filePath}:`, error);
        }
      }
    }
  } else {
