import { BindingNewWatcher, shutdownAsyncRuntime } from '../../binding.cjs';
...
export async function createNewWatcher(
  emitter: WatcherEmitter,
  ...
  const bindingWatcher = new BindingNewWatcher(
    bundlerOptions.map((option) => option.bundlerOptions),
    notifyOptions,
  );
  const watcher = new NewWatcher(
    emitter,
    bindingWatcher,
