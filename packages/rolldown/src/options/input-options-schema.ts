...
import * as valibotExt from '../utils/valibot-ext'
...
const jsxOptionsSchema = v.strictObject({
  ...
  // The rollup preserve is not supported at now
  mode: v.pipe(
...
export const inputOptionsSchema = v.strictObject({
      ...
      v.args(
        v.tuple([
          ...
          v.pipe(
            v.function(),
            v.args(
              v.tuple([LogLevelWithErrorSchema, RollupLogWithStringSchema]),
            ),
          ),
        ]),
      ),
...
export const inputCliOptionsSchema = v.omit(
  v.strictObject({
