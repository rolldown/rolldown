...
export function bindingifyPostBanner(
  args: BindingifyPluginArgs,
): PluginHookWithBindingExt<BindingPluginOptions['postBanner']> {
  const hook = args.plugin.postBanner;
  if (!hook) {
    return {};
  }
...
export function bindingifyPostFooter(
  args: BindingifyPluginArgs,
): PluginHookWithBindingExt<BindingPluginOptions['postFooter']> {
  const hook = args.plugin.postFooter;
  if (!hook) {
    return {};
  }
