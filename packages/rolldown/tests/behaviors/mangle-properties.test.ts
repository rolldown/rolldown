...
test('uses one property mapping across output chunks', async () => {
  ...
  try {
    ...
    expect(testGlobal.crossChunkValue).toBe(42);
  } finally {
...
test('mangles property syntax preserved after TypeScript stripping', async () => {
  const virtualPlugin: Plugin = {
    name: 'virtual',
    resolveId(id) {
    ...
    load(id) {
      if (id === 'entry.ts') {
        return 'class State { _ready: number = 7 } globalThis.rolldownManglePropsResult = new State()._ready;';
      }
    },
  };
...
test('exposes property mangling through normalized output options', async () => {
  let normalizedMinify: unknown;
  const bundle = await rolldown({
    input: 'entry',
    plugins: [
      {
        ...
        load(id) {
          if (id === 'entry') return 'input._shared;';
        },
      },
      {
        name: 'read-normalized-options',
        renderStart(outputOptions) {
          normalizedMinify = outputOptions.minify;
        },
      },
    ],
  });
  ...
  expect(normalizedMinify).toMatchObject({
    mangleProps: {
      include: '^_',
      exclude: '^_skip$',
      reserved: ['_reserved'],
