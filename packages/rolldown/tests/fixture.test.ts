...
function main() {
  ...
  for (const [testConfigPath, testConfig] of Object.entries(testConfigPaths)) {
    ...
    test.skipIf(testConfig.skipComposingJsPlugin)(`${testName}-composing-js-plugin`, async () => {
      testConfig.config = testConfig.config ?? {}
      testConfig.config.experimental = testConfig.config.experimental ?? {}
      testConfig.config.experimental.enableComposingJsPlugins = testConfig.config.experimental.enableComposingJsPlugins ?? true
      try {
        const output = await compileFixture(
          nodePath.join(import.meta.dirname, dirPath),
          testConfig,
