...
// rewrite stands; in `cjs` its `import.meta.url` is polyfilled to
// `require("url").pathToFileURL(__filename).href`, so no warning fires.
export default defineTest({
  ...
  afterTest: (output) => {
    ...
    expect(chunk.code).toContain('require("url").pathToFileURL(__filename).href');
