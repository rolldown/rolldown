...
describe('getLogFilter', () => {
  ...
  it('supports AND conditions', () => {
    ...
    expect(filter({ code: 'FIRST', plugin: 'my-plugin', message: '' })).toBe(
      true,
    );
    expect(filter({ code: 'FIRST', plugin: 'other-plugin', message: '' })).toBe(
      false,
    );
    expect(filter({ code: 'SECOND', plugin: 'my-plugin', message: '' })).toBe(
      false,
    );
  });
  ...
  it('handles numbers and objects', () => {
    ...
    expect(filter({ baz: { a: 1 }, message: '' } as any), 'baz:{"a":1}').toBe(
      true,
    );
    expect(
      filter({ baz: { a: 1, b: 2 }, message: '' } as any),
      'baz:{"a":1,"b":2}',
    ).toBe(
      false,
    );
    expect(
      filter({ baz: { b: 1, c: 2 }, message: '' } as any),
      'baz:{"b":1,"c":2}',
    ).toBe(
      true,
    );
  ...
  it('handles edge case filters', () => {
    ...
    expect(filter({ code: 'A', '': '', message: '' } as any), 'code:A&').toBe(
      true,
    );
    expect(filter({ foo: 'bar:baz', message: '' } as any), 'foo:bar:baz').toBe(
      true,
    );
  });
  ...
  it('handles nested properties', () => {
    ...
    expect(filter({ foo: { bar: 'baz' }, message: '' } as any), 'foo.bar:baz')
      .toBe(true);
    expect(filter({ foo: { bar: 'qux' }, message: '' } as any), 'foo.bar:qux')
      .toBe(false);
    expect(
      filter({ foo: { bar: { baz: 'qux' } }, message: '' } as any),
      'foo.bar.baz:qux',
    ).toBe(false);
  });
});
