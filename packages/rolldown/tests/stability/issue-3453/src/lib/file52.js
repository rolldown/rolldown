export const localStorageDebugFlag = typeof window !== 'undefined' &&
...
export function clampText(text, maxLength) {
  ...
  if (textLength > maxLength) return slicedText + '…';
  return slicedText;
}
...
export function createLogger(showOutput) {
  ...
  function noop() {}
  if (!showOutput) return { log: noop, time: noop, timeEnd: noop };
