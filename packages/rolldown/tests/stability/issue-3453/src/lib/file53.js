...
import { clamp, motion, useAnimate } from 'framer-motion';
...
} from 'https://framerusercontent.com/modules/MWsEnYfRnoOQq31DN4ql/C71JVnvNwV2CrPvApXBB/utils.js';
import { useSearch } from 'https://framerusercontent.com/modules/MyBp84Z0p9nUcMimVMnY/y5F4AGWJ6syeuoB7ebKM/useSearch.js';
...
function ClearButton({ theme, type, onClick, text }) {
  ...
  return /*#__PURE__*/ _jsx('div', {
    style: {
      flexShrink: 0,
      fontSize: theme && theme.titleFont && theme.titleFont.fontSize
  ...
  return /*#__PURE__*/ _jsxs('div', {
    ...
    children: [
      /*#__PURE__*/ _jsx('div', {
        ...
        children: status === 'loading' && inputValue
...
export const ResultRow = /*#__PURE__*/ React.memo(
  /*#__PURE__*/ React.forwardRef(function ResultRow(props, ref) {
    ...
    const borderRadius = isContained
      ...
      : 0;
    const subtitleText = subtitleType === 'path'
      ? urlPath
...
function LayoutContainer(
  { layoutType, theme, onKeyDown, onDismiss, children, modalOptions },
) {
  ...
  const innerStyle = {
    ...
    backgroundColor: layoutType === 'Sidebar'
    ...
    switch (layoutType) {
      case 'FixedTop': {
        ...
        if (prop) return prop;
            ...
            y: -10,
            opacity: .2,
            transition: { duration: Browser.isTouch() ? 0 : .15 },
          };}
        break;
      }
      case 'QuickMenu': {
      ...
      case 'Sidebar': {
        ...
        const prop = modalOptions ? modalOptions[key] : undefined;
        if (prop) return prop;
        else return { x: -10, opacity: 0, transition: { duration: .15 } };
        break;
      }
    }
  }
  ...
  return /*#__PURE__*/ _jsxs('div', {
    ...
    children: [
      layoutType === 'QuickMenu' && /*#__PURE__*/
      _jsx(QuickMenuSpacer, { onClick: onDismiss }),
...
function ModalContainer(
  { layoutType, theme, children, heightIsStatic, heightTransition, heightDeps },
) {
  const style = { // This `willChange` is required to avoid weird rendering issues where
    ...
    maxHeight: layoutType === 'QuickMenu'
...
const ScrollView = /*#__PURE__*/ React.forwardRef(
  function ScrollView({ theme, children }, ref) {
...
function StatusMessage({ status, layoutType, theme }) {
  ...
  const previewInfoText = layoutType === 'FixedTop'
    ? 'Preview Mode'
...
export function SearchModal(props) {
  ...
  const localeId = activeLocale === null || activeLocale === void 0
    ? void 0
    : activeLocale.id;
  const localeSlug = activeLocale === null || activeLocale === void 0
    ? void 0
    : activeLocale.slug;
  const input = useRef();
  ...
  const handleResultRowMouseMove = useCallback((event, index) => {
    setSelected(previousSelected => {
      if (previousSelected.index === index) return previousSelected;
      return { index, scroll: false };
    });
  }, []);
  const router = useRouter();
  const navigateTo = useCallback(async url => {
    if (status === 'no-meta-tag-found') return;
    try {
      var _router_getRoute, _route_page_preload, _route_page, _router_navigate; // Ideally, we would directly expose `routeId` in the search index so that
      // we don't need to infer it from the URL.
      const { routeId, pathVariables } = inferInitialRouteFromPath(
        router.routes,
        url,
      );
      const route = (_router_getRoute = router.getRoute) === null ||
  ...
  const handleKeyDown = event => {
    ...
    switch (event.code) {
      ...
      case 'Escape':
        break;
      case 'Enter':
        if (selectedResult) navigateTo(selectedResult.url);
        break;
      default:
        event.stopPropagation();
    }
  };
  const showNoResults = results.length === 0 && deferredQuery.length > 1 &&
    status !== 'loading';
  const showDivider = Boolean(
    (deferredQuery.length > 0 && results.length > 0 || showNoResults) &&
      status !== 'loading' && props.inputOptions &&
  ...
  const listPaddingTop = showDivider && isItemContained
    ...
    children: [
      /*#__PURE__*/ _jsxs(ModalContainer, {
        ...
        children: [
          ...
          _jsx(Divider, { theme: theme, type: inputOptions.dividerType }),
