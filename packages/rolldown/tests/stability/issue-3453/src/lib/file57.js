...
import { SearchResultTitleType } from 'https://framerusercontent.com/modules/tV9haTHllpHHc9Fjue2H/Xy775u0FJg3K1GQc1Cd9/SearchModal.js';
...
function getNormalizedString(text) {
  if (Array.isArray(text)) return text.map(getNormalizedString);
...
function getMatchRange(currentRange, start, end) {
  ...
  if (end > result.end) result.end = end;
  return result;
} /**
...
function getScoreForSearchIndexItem(item, query, words, fullQuery) {
  ...
  if (
    words.size === 1 && urlWords.size === 1 &&
    urlWords.values().next().value === query
  ) score += score * 5; // Score shorter URLs higher so `/pricing` is before `/lala/pricing`.
  if (score > 0) {
  ...
  if (titleWords.has(query)) score += 10;
  ...
  for (const heading of headings) {
    ...
    if (heading.startsWith(query)) score += 10;
    if (headingWords.has(query)) score += 10;
    if (heading.includes(query)) score += 1; // Fuzzy match full words in headings.
    for (const headingWord of headingWords) {
      const distanceScore = distance(query, headingWord);
      if (distanceScore <= 2) score += 1;
    }
  }
  ...
  for (const p of item.p) if (p.includes(query)) score += .5;
  for (const codeblock of item.codeblock) { // If the full query is close to being the codeblock, score this highly as
    ...
    if (distance(codeblock, fullQuery) <= 2) score *= 10;
    if (codeblock.includes(fullQuery)) score += 10;
    if (codeblock.includes(query)) score += .5;
  }
  return { score, match };
}
...
function useRawSearch(index, query, settings) {
  const results = useMemo(() => {
    ...
    const path = safeWindow === null || safeWindow === void 0
      ? void 0
    ...
    const results = Object.values(index).map(item => {
      ...
      const title = (settings === null || settings === void 0
          ? void 0
...
function getIndexedScopedToUrl(index, rawUrlScope, localeSlug) {
  ...
  for (const url in index) {
    ...
    if (!strippedURL.startsWith(urlScope)) continue;
    if (baseScopeUrlHasVariable && url.length <= urlScope.length) continue;
...
export function useSearch(query, settings) {
  ...
  const localeId = activeLocale === null || activeLocale === void 0
    ? void 0
    : activeLocale.id; // Seperate setter function so that the URL scope is always applied
  ...
  useEffect(() => {
    async function loadSearchIndex() {
      setStatus('loading');
      const metaTag = safeDocument === null || safeDocument === void 0
        ? void 0
      ...
      if (isOverLimit) log('Page limit for plan exceeded'); // If a cached index exists, use the cached version until latest one
