        ...
        () => {
            ...
            try {
              ...
              bundle = bundle ||
                (await rollup({
                  ...
                  onLog: (level, log) => {
                    ...
                    if (
                      level === "warn" &&
                      !config.expectedWarnings?.includes(log.code)
                    ) {
                      warnings.push(log);
                    }
                  },
                  ...
                  plugins: config.options?.plugins?.filter((plugin) =>
                    plugin.name !== "json"
                  ),
              ...
              await generateAndTestBundle(
                bundle,
                {
                  ...
                  validate: true,
                  keepNames: directory.includes(
                      "assignment-to-exports-class-declaration",
                    )
                    ? true
                    : false,
                  ...
                  generatedCode: {
                    preset: "es5",
                    ...config.options?.output?.generatedCode,
                  },
                },
                bundleFile,
                config,
          ...
          if (isSingleFormatTest) {
            return runRollupTest(
              directory + "/_actual.js",
              directory + "/_expected.js",
              "es",
            ).then(
...
async function generateAndTestBundle(
  bundle,
  outputOptions,
  expectedFile,
  { show },
