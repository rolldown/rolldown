 ...
 * served statically.
 *
 * The browser platform no longer goes through this class — it runs on Vite's
 * own full bundle mode (`experimental.bundledDev`); see `vite-server.ts`.
 * Vite's bundled dev is client-environment-only, which is why this node
 * transport (disk output, request gate, stdin rebuild trigger) survives as a
...
export async function createDevServer(
  ...
  if (config.build?.platform === 'browser') {
    return createViteDevServer(config, opts);
  }
...
export async function serve(): Promise<void> {
  ...
  const port = process.env.DEV_SERVER_PORT
    ...
    : devOptions.port;
  if (devConfig.build?.platform === 'browser') {
    // Browser CLI serving runs on Vite bundled dev. (No stdin rebuild
    // trigger here — that channel exists for the node fixtures only.)
    await createViteDevServer(devConfig, { port });
    return;
  }
