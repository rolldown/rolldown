...
export function createDevServerPlugin(
  ...
  return {
    ...
    generateBundle() {
      if (devOptions.platform === 'browser') {
        console.log(
          '[createDevServerPlugin] Generating index.html for browser platform',
        );
  ...
  `;
        if (inputOptions) {
          // FIXME: `cwd` should always be defined, but types say it can be undefined
          let cwd = inputOptions.cwd!;
          const customHtmlPath = nodePath.resolve(cwd, 'index.html');
          if (nodeFs.existsSync(customHtmlPath)) {
            console.log(
              `[createDevServerPlugin] Using custom html file: ${customHtmlPath}`,
            );
