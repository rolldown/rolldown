...
// Ports the retired node fixture `load-dependencies`: a plugin registers `config.json`
// via `this.addWatchFile` in its `load` hook. Editing the watched file — which is not a
// module in the graph — must re-load the watching module and hot-apply it through the
// self-accept, not full-reload.
