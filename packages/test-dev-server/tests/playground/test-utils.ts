...
export async function errorOverlayText(): Promise<string> {
  // One atomic evaluate: a `count()`-then-`locator.evaluate()` pair races
  // overlay teardown — `evaluate` auto-waits (default 30s) on an element that
  // detached between the two calls instead of returning `''`.
